...
void main()
{
    ...
    float amplitude = sin(time * 10.0 + distance * 20.0) * 0.05;

    
    vec3 displacedPosition = position + normals * amplitude;
...
void main()
{
    ...
    float wave = sin(outPosition.x * 5.0 + time * 2.0) + sin(outPosition.y * 5.0 + time * 2.5) + sin(outPosition.z * 5.0 + time * 3.0);
    float colorFactor = wave / 3.0;

    
    vec3 rainbowColor = vec3(
        sin(colorFactor * 3.0 + 0.0),
        sin(colorFactor * 3.0 + 2.0),
        sin(colorFactor * 3.0 + 4.0)
    );

   
    rainbowColor = rainbowColor * 0.5 + 0.5;

    
    vec4 texColor = texture(tex, outTexCoords);
    vec3 finalColor = mix(texColor.rgb, rainbowColor, 0.7);

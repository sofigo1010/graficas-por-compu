...
class Star(Shape):
    def __init__(self, position, size, material):
        ...
        self.size = size
        self.type = "Star"
        self.triangles = self.generate_star()

    def generate_star(self):
        triangles = []
        radius_outer = self.size
        radius_inner = self.size * 0.5
        
        angle = 72  
        for i in range(5):

            outer1 = [
                self.position[0] + radius_outer * math.cos(math.radians(i * angle)),
                self.position[1] + radius_outer * math.sin(math.radians(i * angle)),
                self.position[2]
            ]
            outer2 = [
                self.position[0] + radius_outer * math.cos(math.radians((i + 1) * angle)),
                self.position[1] + radius_outer * math.sin(math.radians((i + 1) * angle)),
                self.position[2]
            ]
            

            inner = [
                self.position[0] + radius_inner * math.cos(math.radians(i * angle + angle / 2)),
                self.position[1] + radius_inner * math.sin(math.radians(i * angle + angle / 2)),
                self.position[2] - radius_inner * 0.5  
            ]
            

            triangles.append(Triangle(self.position, outer1, inner, self.material))
            triangles.append(Triangle(self.position, inner, outer2, self.material))
...
class Hemisphere(Shape):
    def __init__(self, position, radius, material, orientation='up'):
    ...
    def ray_intersect(self, orig, dir):
        ...
        if self.orientation == 'up' and P[1] < self.position[1]:
            return None 
        if self.orientation == 'down' and P[1] > self.position[1]:
            return None  
...
class Ellipsoid(Shape):
    ...
    def ray_intersect(self, orig, dir):
        ...
        ox, oy, oz = [(orig[i] - self.position[i]) / self.radii[i] for i in range(3)]
        dx, dy, dz = [dir[i] / self.radii[i] for i in range(3)]
        ...
        P = [orig[i] + dir[i] * t0 for i in range(3)]
        normal = [(P[i] - self.position[i]) / (self.radii[i] ** 2) for i in range(3)]
        norm_magnitude = math.sqrt(sum([normal[i] ** 2 for i in range(3)]))
        normal = [normal[i] / norm_magnitude for i in range(3)]

...
class Renderer(object):
    ...
    def glDrawTrianglePoint(self, A, B, C, P):
        ...
        # Hay que asegurarse que la suma de coordenadas
        # baricentricas es igual a 1

        if not math.isclose(u+v+w, 1.0):
            return
        
        # se calcula el valor de Z de este pixel específico
        z = u * A[2] + v * B[2] + w * C[2]

        # Sie l valor de Z para este punto es mayor que el valor guardado
    ...
    def glDrawPrimitives(self, buffer, vertexOffset):
        ...
        # toda la información de un vértice (posición, coordenadas
        # de textura, normales, color, etc.). El VertexOffset se
        # refiere a cada cuántos valores empieza la información
        # de un vértice individual.
        # Se asume que los primeros tres valores de un vértice
        # corresponden a Posición.
        
        if self.primitiveType == POINTS:
            # Si son puntos, revisamos el buffer en saltos igual
            # al Vertex Offset. El valor X y Y de cada vértice
            ...
            # Si son líneas, revisamos el buffer en saltos igual
            # a 3 veces el Vertex Offset, porque cada trío corresponde
            # a un triángulo.
            for i in range(0, len(buffer), vertexOffset * 3):
                for j in range(3):
                    # Hay que dibujar la línea de un vértice al siguiente
                    x0 = buffer[i + vertexOffset * j + 0]
                    y0 = buffer[i + vertexOffset * j + 1]
                    
                    # En caso de que sea el último vértice, el siguiente
                    # sería el primero
            ...
            # Si son triángulos revisamos el buffer en saltos igual
            # a 3 veces el Vertex Offset, porque cada trío corresponde
            # a un triángulo.
            for i in range(0, len(buffer), vertexOffset * 3):
                # Necesitamos tres vértices para mandar a dibujar el triángulo.
                # Cada vértice necesita todos sus datos, la cantidad de estos
